    ...
    if 'DATABASE_URL' in os.environ:
        ...
        try:
            ...
            conn = psycopg2.connect(
                ...
                host=result.hostname, port=result.port, connect_timeout=10)
...
def load_registers():
    ...
    # We will clear transactions to perform a clean load
    print("Clearing old transaction data for a clean import...")
    ...
    # Insert a fallback donor in case referenced IDs are missing
    ...
    for year_prefix, filename in files_map.items():
        ...
        print(f"Opening {filename} for year {year_prefix}...")
            ...
            if len(rows) > 1:
                ...
                if donors:
                    q = """
                        ...
                        ON CONFLICT (Shop_Donor_ID) DO NOTHING
        ...
        if 'Bills' in wb.sheetnames:
            ...
            if len(rows) > 1:
                ...
                for r in rows[1:]:
                    ...
                    if not shop_id or not bill_date or not bill_no:
                        continue
        ...
        if 'Indents' in wb.sheetnames:
            ...
            if len(rows) > 1:
                ...
                for r in rows[1:]:
                    ...
                    if not code or not inst_id or not qty or not indent_no:
                        continue
                ...
                if indents:
                    ...
                    conn.commit()
                    print(f"  Loaded {len(indents)} indents.")
        ...
        if 'Stock' in wb.sheetnames:
            ...
            if len(rows) > 1:
                ...
                for r in rows[1:]:
                    ...
                    # Try to map shop_donor_id from Shop_Donor_Name
                    # For simplicity, we fallback to our general fallback ID
                    donor_id = placeholder_donor_id
                    
                    inwards.append((
                        year_prefix, date1, code, donor_id, book_no, receipt_no, receipt_date, '', '', rate, amt, 'No', stock_qty, None, 0.0, 0.0, 'Central Godown', datetime.date.today().strftime('%Y-%m-%d'), 'Imported', purch_don
                    ))
                ...
                if inwards:
                    ...
                    """ if db_type == "postgresql" else """
                        ...
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """
                    # Chunk insert to prevent database locks or driver memory limits
                    chunk_size = 1000
                    for idx in range(0, len(inwards), chunk_size):
                        cursor.executemany(q, inwards[idx : idx + chunk_size])
                        conn.commit()
                    print(f"  Loaded {len(inwards)} stock inwards.")
        ...
        if 'Issue' in wb.sheetnames:
            ...
            if len(rows) > 1:
                ...
                for r in rows[1:]:
                    ...
                    if not code or not issue_qty or not inst_id:
                        continue
                ...
                if outwards:
                    ...
                    for idx in range(0, len(outwards), chunk_size):
                        ...
                        conn.commit()
                    print(f"  Loaded {len(outwards)} stock issues.")
        ...
        if 'Vegetable' in wb.sheetnames:
            ...
            if len(rows) > 1:
                ...
                for r in rows[1:]:
                    ...
                    if not inst_id or not qty or not v_code:
                        continue
                ...
                if vegs:
                    ...
                    for idx in range(0, len(vegs), chunk_size):
                        ...
                        conn.commit()
                    print(f"  Loaded {len(vegs)} vegetable logs.")
                    
    conn.close()
    print("All Excel registers loaded successfully into the database!")
